 ...
 * 获取主流加密货币实时价格 (人民币 CNY)，根据 widgetFamily 生成不同 UI
 */

var COINS = "bitcoin,ethereum,cardano,ripple,tron,binancecoin,shiba-inu,basic-attention-token";
var API_URL = "https://api.coingecko.com/api/v3/simple/price?ids=" + COINS + "&vs_currencies=cny&include_24hr_change=true";
...
var COIN_MAP = {
  ...
  tron:                    { symbol: "TRX",  name: "Tron",      icon: "t.circle.fill",            color: "#FF060A" },
  binancecoin:             { symbol: "BNB",  name: "BNB Chain", icon: "hexagon.fill",             color: "#F3BA2F" },
  "shiba-inu":             { symbol: "SHIB", name: "Shiba Inu", icon: "pawprint.fill",            color: "#FFA409" },
  "basic-attention-token": { symbol: "BAT",  name: "BAT",       icon: "triangle.fill",            color: "#FF5000" },
};
...
function formatPrice(price) {
  ...
  if (price < 0.0001) return "¥" + price.toFixed(8); // 特别为 SHIB 增加高精度小数显示
...
function coinCard(id, data, variant) {
  ...
  var change = data.cny_24h_change;
  ...
  if (p.layout === "column") {
    return vstack([
      ...
      txt(formatPrice(data.cny), p.priceSize, "semibold", "#FFFFFF", { minScale: 0.6, maxLines: 1 }),
      changeRow,
    ], cardOpts);
  }
  ...
  return vstack([
    hstack([
      ...
      spacer(),
      vstack([
        txt(formatPrice(data.cny), p.priceSize, "bold", "#FFFFFF"),
        changeRow,
...
function coinRow(id, data, compact) {
  ...
  return hstack([
    ...
    spacer(),
    txt(formatPrice(data.cny), sz, "semibold", "#FFFFFF", { maxLines: 1, minScale: 0.7 }),
...
function buildAccessoryCircular(prices) {
  ...
  var change = btc ? btc.cny_24h_change : 0;
  return {
    type: "widget",
    gap: 2,
    children: [
      ...
      txt(btc ? formatPrice(btc.cny) : "--", 12, "bold", null, { minScale: 0.5 }),
...
function buildAccessoryRectangular(prices) {
  var ids = filterAvailable(["bitcoin", "ethereum", "cardano", "ripple"], prices);
  var rows = ids.map(function (id) {
    ...
    return hstack([
      ...
      spacer(),
      txt(formatPrice(data.cny), 10, "semibold", null, { minScale: 0.5, maxLines: 1 }),
...
function buildAccessoryInline(prices) {
  ...
  var text = "";
  if (btc) text += "BTC " + formatPrice(btc.cny) + " " + formatChange(btc.cny_24h_change);
  else text += "BTC --";
  if (eth) text += " | ETH " + formatPrice(eth.cny);
  return {
    type: "widget",
    children: [
...
function buildSystemSmall(prices) {
  var rows = filterAvailable(["bitcoin", "ethereum", "cardano", "ripple"], prices)
...
function buildSystemExtraLarge(prices) {
  var featured = filterAvailable(["bitcoin", "ethereum", "cardano"], prices)
